...
##
  ...
  listMapOutput[[i + 1]] = ggmap(map) + geom_point(data = mapData, mapping = aes(x = longitude, y = latitude, color = factor(location)), size = .1) + theme(legend.position = "bottom")+ ggtitle(paste0("Top ",locationCount," locations between ",decades[i]-10, " and ", decades[i]))
...
intersectionBetweenDecades <- function(dataframe, locationCount){
  ...
  Reduce(intersect, listOutput[-c(1:5,10:11)])
  #return(listOutput)
}
