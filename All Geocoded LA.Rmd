...
geocodedAllAttemtpt %>% filter(is.na(latitude))
```
...
hist(la$Year)
hist(geocodedLAReDo$Year)
geocodedLAFound %>% filter(Year < 1930) %>% filter(!is.na(latitude))
...
Take the redo ones and send them through again but with a general tag and not Lost Angeles California.

```{r}
geocodedLAReDo <- geocodedLAReDo %>% unite(Address, c("Hse.No","Street","Area"), sep = " ",remove = FALSE)
geocodedLAReDo$Address <- paste(geocodedLAReDo$Address, ", CA")
geocodedLAReDo
```


```{r}
geocodedAllCali25 <- tidygeocoder::geocode(.tbl = geocodedLAReDo[1:25000,], address = Address, method = "osm", lat = latitude, long = longitude)
