...
#ggmap(map) + geom_point(data = detroitData, mapping = aes(x = longitude, y = latitude, color = factor(location)), size = 1) + theme(legend.position = "none")
...
detroitData <- detroitData %>% filter(! (latitude < 42.4 & latitude > 42.35 & longitude > -83.0 & longitude < -83.1) )
detroitData
