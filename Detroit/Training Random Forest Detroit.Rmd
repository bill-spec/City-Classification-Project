...
tbl <- cleanDetroit %>% filter(!( (latitude > 42.30 & latitude < 42.4)  & (longitude < -83.0 & longitude > -83.1)) ) %>% group_by(location) %>% summarise(count = n()) %>% arrange(desc(count)) %>% filter(location != "")
...
```{r}
detroitAddress <- cleanDetroit %>% filter(location %in% listOfAreas) %>% filter(!( (latitude > 42.30 & latitude < 42.4)  & (longitude < -83.0 & longitude > -83.1)) )
